@include ('admin.assets.headers')

    <body class="layout-app layout-sticky-subnav ">
        @livewireStyles
        <div class="preloader">
            <div class="sk-chase">
                <div class="sk-chase-dot"></div>
                <div class="sk-chase-dot"></div>
                <div class="sk-chase-dot"></div>
                <div class="sk-chase-dot"></div>
                <div class="sk-chase-dot"></div>
                <div class="sk-chase-dot"></div>
            </div>
        </div>
         <!--New Task Model-->
                        @include ('admin.creatives.layout.newtaskmodal')
         <!--pop-up for active  -->
                      @include ('admin.creatives.layout.viewpop')


                        
               
        

            <!-- <div class="sk-bounce">
    <div class="sk-bounce-dot"></div>
    <div class="sk-bounce-dot"></div>
  </div> -->

            <!-- More spinner examples at https://github.com/tobiasahlin/SpinKit/blob/master/examples.html -->
        

        <div class="mdk-drawer-layout js-mdk-drawer-layout"
             data-push
             data-responsive-width="992px">
            <div class="mdk-drawer-layout__content page-content">

                <!-- Header -->
                 @include ('admin.assets.headnavbar')
                <!-- // END Header -->
               
                <div class="border-bottom-2 py-32pt position-relative z-1">
                    <div class="container-fluid page__container d-flex flex-column flex-md-row align-items-center text-center text-sm-left">
                        <div class="flex d-flex flex-column flex-sm-row align-items-center mb-24pt mb-md-0">

                            <div class="mb-24pt mb-sm-0 mr-sm-24pt">
                                <h2 class="mb-0">Tasks</h2>

                                <ol class="breadcrumb p-0 m-0">
                                    <li class="breadcrumb-item"><a href="index.html">Home</a></li>

                                    <li class="breadcrumb-item ">

                                        Creatives Team 

                                    </li>

                                     <li class="breadcrumb-item active">

                                        Layout Service  

                                    </li>

                                </ol>

                            </div>
                        </div>

                         <div class="row" role="tablist">
                            <div class="col-auto border-left">
                                <button type="button"
                                        class="btn btn-accent"
                                        data-toggle="modal"
                                        data-target="#photoModal">
                                    New Task
                                </button>
                            </div>
                        </div>
                        
                    </div>
                </div>

                <div class="container-fluid page__container">
                    <div class="page-section">

                    <!--overview-->
                    @include ('admin.creatives.layout.overview')
                    <!--endoverview-->

                    <!--overview-->
                    @include ('admin.creatives.layout.table.team')
                    <!--endoverview-->

                        <div class="page-separator">
                            <div class="page-separator__text">Tasks</div>
                        </div>

                        <div class="card mb-0 p-relative o-hidden">
                            <div class="card-header py-12pt d-flex align-items-center">
                                <strong>Request</strong>
                                <div class="text-50">Total:{{$totalCount}}</div>
                                <div class="flex"></div>
                                <a href="#"><i class="material-icons text-20">keyboard_arrow_down</i></a>
                            </div>
                           
                        </div>

                         <div class="card dashboard-area-tabs p-relative o-hidden mb-lg-32pt">
                        <div class="card-header p-0 nav">
                            <div class="row no-gutters"
                                 role="tablist">
                                <!-- Active Counter -->
                                <div class="col-auto">
                                    <a href="#active" data-toggle="tab" role="tab" aria-selected="true"
                                    class="dashboard-area-tabs__tab card-body d-flex flex-row align-items-center justify-content-start active">
                                        <span class="h2 mb-0 mr-3" id="count-active">{{ $inProgressCount }}</span>
                                        <span class="flex d-flex flex-column">
                                            <strong class="card-title">Active</strong>
                                            <small class="card-subtitle text-50">Ongoing Projects</small>
                                        </span>
                                    </a>
                                </div>

                                <!-- Archive Counter -->
                                <div class="col-auto border-left border-right">
                                    <a href="#arch" data-toggle="tab" role="tab" aria-selected="false"
                                    class="dashboard-area-tabs__tab card-body d-flex flex-row align-items-center justify-content-start">
                                        <span class="h2 mb-0 mr-3" id="count-archive">{{ $postedCount }}</span>
                                        <span class="flex d-flex flex-column">
                                            <strong class="card-title">Archive</strong>
                                            <small class="card-subtitle text-50">Finished Projects</small>
                                        </span>
                                    </a>
                                </div>

                                <!-- Month Counter -->
                                <div class="col-auto border-left border-right">
                                    <a href="#month" data-toggle="tab" role="tab" aria-selected="false"
                                    class="dashboard-area-tabs__tab card-body d-flex flex-row align-items-center justify-content-start">
                                        <span class="h2 mb-0 mr-3" id="count-month">{{ $monthCount }}</span>
                                        <span class="flex d-flex flex-column">
                                            <strong class="card-title">This month</strong>
                                            <small class="card-subtitle text-50">Past Projects</small>
                                        </span>
                                    </a>
                                </div>

                                <!-- Total Counter -->
                                <div class="col-auto border-left border-right">
                                    <a href="#list" data-toggle="tab" role="tab" aria-selected="false"
                                    class="dashboard-area-tabs__tab card-body d-flex flex-row align-items-center justify-content-start">
                                        <span class="h2 mb-0 mr-3" id="count-total">{{ $totalCount }}</span>
                                        <span class="flex d-flex flex-column">
                                            <strong class="card-title">List</strong>
                                            <small class="card-subtitle text-50">All Projects</small>
                                        </span>
                                    </a>
                                </div>

                                                            

                                                            </div>

                                                        </div>

                                                    <div class="tab-content">

                                    {{-- ACTIVE PROJECTS --}}
                                    <div class="tab-pane fade show active"
                                        id="active"
                                        role="tabpanel">
                                        <div class="table-responsive">
                                            @include ('admin.creatives.layout.table.active')
                                        </div>
                                    </div>

                                    {{-- THIS MONTH PROJECTS --}}
                                    <div class="tab-pane fade"
                                        id="month"
                                        role="tabpanel">
                                        <div class="table-responsive">
                                            @include ('admin.creatives.layout.table.month')
                                        </div>
                                    </div>

                                    {{-- ALL PROJECTS (LIST) --}}
                                    <div class="tab-pane fade"
                                        id="list"
                                        role="tabpanel">
                                        <div class="table-responsive">
                                            @include ('admin.creatives.layout.table.list')
                                        </div>
                                    </div>

                                    {{-- ARCHIVED PROJECTS --}}
                                    <div class="tab-pane fade"
                                        id="arch"
                                        role="tabpanel">
                                        <div>
                                            @include ('admin.creatives.layout.table.archive')
                                        </div>
                                    </div>
                                
                                </div>

     <script>
    document.addEventListener("DOMContentLoaded", function () {
        // 1. Check if there's a saved tab in localStorage and activate it
        let activeTab = localStorage.getItem('activeDashboardTab');
        if (activeTab) {
            let tabTrigger = document.querySelector(`a[href="${activeTab}"]`);
            if (tabTrigger) {
                // Use Bootstrap's tab trigger if available, or fallback to click()
                if (typeof bootstrap !== 'undefined' && bootstrap.Tab) {
                    var tab = new bootstrap.Tab(tabTrigger);
                    tab.show();
                } else {
                    tabTrigger.click();
                }
            }
        }

        // 2. Save the tab href to localStorage whenever a tab is clicked
        const tabs = document.querySelectorAll('.dashboard-area-tabs__tab');
        tabs.forEach(tab => {
            tab.addEventListener('click', function (e) {
                let targetID = this.getAttribute('href');
                localStorage.setItem('activeDashboardTab', targetID);
            });
        });
    });
</script>  
 <script>
  document.addEventListener("DOMContentLoaded", function () {
    setInterval(function() {
        if (document.hidden) return;

        // Find which tab is currently active
        let activeTabPane = document.querySelector('.tab-pane.active, .tab-pane.show.active');
        if (!activeTabPane) return;

        let activeTabId = activeTabPane.getAttribute('id'); // e.g., 'active', 'month', 'list', 'archive'

        fetch(window.location.href, {
            headers: {
                'X-Requested-With': 'XMLHttpRequest'
            }
        })
        .then(response => response.text())
        .then(html => {
            let parser = new DOMParser();
            let doc = parser.parseFromString(html, 'text/html');
            
            // 1. Update the active tab's table content
            let newPane = doc.querySelector(`#${activeTabId}`);
            if (newPane) {
                activeTabPane.innerHTML = newPane.innerHTML;

                // If List.js is running on the archive tab, trigger a resize/refresh event
                if (activeTabId === 'archive') {
                    window.dispatchEvent(new Event('resize'));
                }
            }

            // 2. Update all the counters dynamically
            const counters = ['count-active', 'count-archive', 'count-month', 'count-total'];
            counters.forEach(id => {
                let element = document.getElementById(id);
                let newElement = doc.getElementById(id);
                if (element && newElement) {
                    element.textContent = newElement.textContent;
                }
            });
        })
        .catch(error => console.error('Error updating dashboard data:', error));
    }, 5000); // 5000ms = 5 seconds
});
</script>

                    </div>

                    </div>
                </div>

            </div>
            <!-- // END drawer-layout__content -->

            <!-- drawer -->
             @include ('admin.assets.sidebar')
            <!-- // END drawer -->
        </div>
        <!-- // END drawer-layout -->

        <!-- App Settings FAB -->
         @include ('admin.assets.footer')
    </body>
@if(session('success') || session('error') || $errors->any())
    <script>
        @if(session('success'))
            Swal.fire({
                icon: 'success',
                title: 'Good job!',
                text: "{{ session('success') }}",
                confirmButtonText: 'OK'
            });
        @elseif(session('error'))
            Swal.fire({
                icon: 'error',
                title: 'Saving Failed',
                text: "{{ session('error') }}",
                confirmButtonText: 'OK'
            });
        @elseif($errors->any())
            Swal.fire({
                icon: 'error',
                title: 'Validation Error!',
                html: '{!! implode("<br>", $errors->all()) !!}',
                confirmButtonText: 'OK'
            });
        @endif
    </script>
@endif
</html>